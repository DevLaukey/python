...
def generate_maze(size=10, density=0.2):
    ...
    while maze[start_row][start_col] != '.' or maze[goal_row][goal_col] != '.':
        # Retry until valid positions for 'S' and 'G' are found
        start_row, start_col = random.randint(0, size - 1), random.randint(0, size - 1)
        goal_row, goal_col = random.randint(0, size - 1), random.randint(0, size - 1)
...
def breadth_first_search(maze, start, goal):
    ...
    # Reconstruct the path from goal to start if goal is reachable
    if goal in parent:
        path = []
        current = goal
        while current != start:
...
def evaluate_algorithm(algorithm, maze, start, goal):
    ...
    execution_time = None  # Initialize execution_time to None
    ...
    try:
        if algorithm == "DFS":
            ...
            astar_search(maze, start, goal)
    except Exception as e:
        print(f"Error in {algorithm}: {e}")
    finally:
        end_time = time.time()
        if execution_time is not None:  # Check if execution_time is set
            execution_time = end_time - start_time
            print(f"Execution Time: {execution_time:.5f} seconds")

        expanded_nodes = count_expanded_nodes(algorithm, maze, start, goal)
        print(f"Number of Nodes Expanded: {expanded_nodes}")
        
            
def count_expanded_nodes(algorithm, maze, start, goal):
    visited = set()
    expanded_nodes = 0
    ...
    if algorithm == "DFS":
        ...
        while queue:
            ...
            for neighbor_row, neighbor_col in neighbors:
                if 0 <= neighbor_row < len(maze) and 0 <= neighbor_col < len(maze[0]) and maze[neighbor_row][neighbor_col] == '.' and (neighbor_row, neighbor_col) not in visited:
                    queue.append((neighbor_row, neighbor_col))
                    visited.add((neighbor_row, neighbor_col))
                    expanded_nodes += 1
        ...
        while open_set:
            current = min(open_set, key=lambda node: heuristic(node, goal))
            open_set.remove(current)
...
def compare_algorithms():
    ...
    for algorithm in algorithms:
        print(f"\n--- {algorithm} ---")
        execution_time = evaluate_algorithm(algorithm, maze, start_point, goal_point)
        print(f"Execution Time: {execution_time:.5f} seconds")

        expanded_nodes = count_expanded_nodes(algorithm, maze, start_point, goal_point)
        print(f"Number of Nodes Expanded: {expanded_nodes}")
