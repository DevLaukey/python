...
def generate_maze(size, density):
    ...
    visited = set()
    walls = [(start[0] + 1, start[1]), (start[0], start[1] + 1)]
    visited.add(start)
    ...
    while walls:
        ...
        adjacent_cells = [(wall[0] + 1, wall[1]), (wall[0] - 1, wall[1]), (wall[0], wall[1] + 1), (wall[0], wall[1] - 1)]
        adjacent_cells = [cell for cell in adjacent_cells if is_valid(cell) and cell not in visited]

        if len(adjacent_cells) == 1:
            maze[wall[0]][wall[1]] = ' '
            visited.add(wall)
            walls.extend(adjacent_cells)
